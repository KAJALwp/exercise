...
class Blocks {
	...
	protected function setup_hooks() {
		...
		add_filter( 'block_categories_all', array( $this, 'opry_custom_block_category' ) );
	...
	public function register_theme_blocks() {
		...
		if ( file_exists( OPRY_SRC_BLOCK_DIR_PATH ) ) {
			$block_json_files = glob( OPRY_SRC_BLOCK_DIR_PATH . '/*/block.json' );
	 ...
	 * @since 1.0.0
	 */
	public function opry_custom_block_category( $categories ) {
		return array_merge(
			array(
				array(
					'slug'  => 'opry',
	...
	public static function block_class_from_string( string $string ): ?string {
		...
		if ( false !== strpos( $string, 'opry/' ) ) {
			$string = str_replace( 'opry/', '', $string );
		}
