...
namespace OPRY\Includes\Helpers;
...
function autoloader( $resource = '' ) {
	...
	if ( 'includes' === $path[0] ) {
		...
		$resource_path = sprintf( '%s/includes/%s/%s.php', untrailingslashit( OPRY_THEME_PATH ), $directory, $file_name );
	...
	if ( 'blocks' === $path[0] && ! empty( $path[1] ) ) {
		$block_dir_path = OPRY_THEME_PATH . '/assets/src/blocks/' . $path[1];
...
spl_autoload_register( '\OPRY\Includes\Helpers\autoloader' );
